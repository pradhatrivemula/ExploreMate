...
namespace JSONWebAPI
{
    public class ServiceAPI : IServiceAPI
    {
        ...
        public string addfinalroute(string id, string uid, string placeno, string name, string cont, string lat, string lng, string add, string dist, string isopen, string opentill, string rating, string review, string price, string time_frame, string date,string latlng)
        {
            string s = "false";
            name = name.Replace('\'', ' ');
            ...
            try
            {
                ...
                cmd = new SqlCommand("update Cust set latlng='" + latlng + "' where Id='" + uid + "'", con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
        ...
        public string getcurrentroute(string uid, string date)
        {
            ...
            try
            {
                ...
                if (s.Contains("*"))
                {
                    cmd = new SqlCommand("select latlng from Cust where Id='" + uid + "'", con);
                    con.Open();
                    dr=cmd.ExecuteReader();
                    if (dr.HasRows)
                    {
